...
function checkPassword(value, min, max, errorId) {
  ...
  if (regexPassword.test(value)) {
    ...
    getElement(
      errorId
    ).innerHTML = `Mật khẩu phải từ ${min} - ${max} ký tự, có ít nhất 1 số, 1 chữ in hoa, 1 ký tự đặc biệt`;
    return false;
  }
}
...
function checkSalary(value, min, max, errorId) {
  ...
  if (regexNumber.test(value)) {
    if (value >= min * mil && value <= max * mil) {
      ...
      getElement(errorId).style.display = "block";
      getElement(errorId).innerHTML = `Lương cơ bản nằm trong khoảng ${
        min * mil
      } - ${max * mil}`;
      return false;
    }
...
function checkTimeWork(value, min, max, errorId) {
  ...
  if (regexNumber.test(value)) {
    if (value >= min && value <= max) {
      ...
      getElement(
        errorId
      ).innerHTML = `Số giờ làm trong tháng nằm trong khoảng ${min} - ${max} giờ`;
      return false;
    }
  } else {
    getElement(errorId).style.display = "block";
...
function checkDuplicate(value, arr, key, errorId, action) {
    ...
    case "them":
      for (var i = 0; i < arr.length; i++) {
        if (arr[i][key] == value) {
          no_duplicate = false;
          getElement(errorId).style.display = "block";
          getElement(errorId).innerHTML = message[5];
          return no_duplicate;
        }
      }
